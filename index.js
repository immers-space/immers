...
app.post(
  '/auth/user',
  // optional client login for registration setting override
  passport.authenticate(['oauth2-client-jwt', 'anonymous'], { session: false }),
  settings.isTrue(
    'enablePublicRegistration',
    req => req.user?.canControlUserAccounts
  ),
  auth.validateNewUser,
  auth.logout,
  registerActor,
  auth.registration
)
